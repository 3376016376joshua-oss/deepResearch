  ...
  install(instance) {
    instance.interceptors.response.use(
      (response) => {
        ...
        if (!isServiceStatus(data[CODE_KEY])) return response
      ...
      (error) => {
        ...
        if (!isServiceStatus(data[CODE_KEY])) return Promise.reject(error)
...
function isServiceStatus(value: unknown): value is 'success' | 'error' {
