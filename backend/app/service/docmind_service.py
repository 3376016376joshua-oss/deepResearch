...
class DocMindService:
    ...
    def __init__(self):
        ...
        if not self.access_key_id or not self.access_key_secret:
            print(
                "[DocMind] 凭证未完整配置: "
                f"access_key_id_exists={bool(self.access_key_id)}, "
                f"access_key_secret_exists={bool(self.access_key_secret)}"
            )
    ...
    def submit_job(self, file_path: str, file_name: str) -> Optional[str]:
        ...
        try:
            ...
            print(
                "[DocMind] 提交任务: "
                f"file_name={file_name}, file_ext={file_ext}, file_path={file_path}"
            )
            ...
            if isinstance(body_map, dict):
                ...
                message = body_map.get("message") or body_map.get("Message")
                request_id = body_map.get("request_id") or body_map.get("RequestId")
                print(
                    "[DocMind] 提交响应: "
                    f"code={code}, "
                    f"message={message}, "
            ...
            print(f"[DocMind] 提交任务未返回有效 task_id，完整响应: {body_map}")
            return None

        except Exception as e:
            print(f"[DocMind] 提交任务异常: {e}")
            import traceback
            traceback.print_exc()
            return None
